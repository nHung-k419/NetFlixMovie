...
function Movie() {
  ...
  return (
    ...
    <div className='w-full h-[1200px] bg-slate-800'>
      {Movie.map((item, index) => (
        <div key={index}>
          ...
          <div className='lg:pl-[75px] mt-6 pl-7'>
            <h2 className='font-bold text-yellow-500 text-2xl'>Nội Dung</h2>
            <p className='text-white text-lg font-bold text-justify w-[93%]'>{item.content}</p>
            <div className='font-bold text-yellow-500 text-2xl mt-3 flex'>Quốc Gia : <span className='text-2xl text-white ml-2'>{item.country[0].name}</span></div>
            <div className='font-bold text-yellow-500 text-2xl mt-3 flex'>Thời Gian : <span className='text-2xl text-white ml-2'>73 Phút</span></div>
            <ul className='font-bold text-yellow-500 text-2xl mt-3 '>Đạo Diễn
              <li className='text-xl text-white ml-2'>- {item.director}</li>
            </ul>
            <ul className='font-bold text-yellow-500 text-2xl mt-3 '>Diễn Viên
              {item.actor.map((actor, index) => (
                <li key={index} className='text-xl text-white ml-2'>- {actor}</li>
              ))}

            </ul>
            <ul className='font-bold text-yellow-500 text-2xl mt-3 '>Thể Loại
