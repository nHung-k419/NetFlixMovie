...
import { handleGetPageCategories, handleSavemovie } from '../Service';
function Category_Movie() {
    ...
    useEffect(() => {
        const CallPageCategories = async () => {
            const data = await handleGetPageCategories(slug_cate, Page)
            setPartSeriesMovie(data.data.items)
            setTitle_Movie(data.data.titlePage)
